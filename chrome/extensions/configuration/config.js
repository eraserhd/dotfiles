...
const hint = ({ selectors = '*', lock = false } = {}) => {
  const hint = new Hint
...
modal.map('Command', ['Shift', 'KeyF'], () => hint({ lock: true }).start(), 'Select multiple links')
...
modal.map('Command', ['Shift', 'KeyY'], () => { Clipboard.copy(`[${document.title}](${location.href})`); notify('Page address and title copied') }, 'Copy page address and title')
...
modal.map('Link', ['Shift', 'KeyY'], () => { copy((element) => `[${element.textContent}](${element.href})`); notify('Link address and text copied') }, 'Copy link address and text')
...
modal.map('Link', ['KeyM'], () => mpv(), 'Play with mpv')
modal.map('Link', ['Alt', 'KeyM'], () => mpv({ reverse: true }), 'Play with mpv in reverse order')
