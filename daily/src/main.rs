...
fn main() {
    Command::new("osascript").arg("-e").arg(ANKI_SCRIPT)
        .status()
        .expect("failed to run Anki script");

    Command::new("open").arg("/System/Library/CoreServices/Menu Extras/Bluetooth.menu")
        .status()
        .expect("failed to open bluetooth menu");

    Command::new("osascript").arg("-e").arg(COMPLETE_SCRIPT)
        .status()
        .expect("failed to check off 'Run Daily' task");

    Command::new("git").arg("add").arg("-A")
        .current_dir("/Users/jfelice/src/data")
        .status()
        .expect("failed to `git add -A` in data");

    Command::new("git").arg("commit").arg("-m").arg("daily")
        .current_dir("/Users/jfelice/src/data")
        .status()
        .expect("failed to `git commit` in data");

    Command::new("git").arg("push").arg("origin").arg("master")
        .current_dir("/Users/jfelice/src/data")
        .status()
        .expect("failed to `git push` in data");

    Command::new("gtypist").arg("-b").arg("/Users/jfelice/src/dotfiles/gtypist/gtypist.typ")
        .status()
